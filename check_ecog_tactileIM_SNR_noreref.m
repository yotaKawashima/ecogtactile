...
%     end
    ...
    format_fig;
    plot(faxis,squeeze(mean(snr_bych(:,:,:),2))','Color','k')
    c=0;
    hold on;
    for n=theseFreqs
        c=c+1;
        [~,idxfreqs(c)]=findclosest(faxis,n);
        tp=squeeze(mean(snr_bych(:,:,idxfreqs(c)),2));
        if fillCode(c)==1
...
xlim([2 45])
   ...
   elecR=6;
    ...
    imagesc(1:elecC,1:elecR,reshape(mean(stats.tstat,3),elecC,elecR)'); set(gca,'ydir','normal');
