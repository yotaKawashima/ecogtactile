...
preDataPath = '/Users/tand0009/Data/ECoG_tactile/S2/attention/';
chronuxPath = [rootFile 'Work/local/chronux_2_12/'];

addpath(genpath(filePath));
allversions={'1a','1arev'};
...
for nversion=1:length(allversions) %was 4:5
    ...
    elseif strcmp(version,'1arev') % version 1a: sensors on both hands, frequencies flipped compared to 1a
        data_path= [preDataPath];
        file_name= 'HandsNormal_InverseFrequencies.mat';
        LeftF0=[23 17];
        RightF0=[11 8];
    end
...
%%
erp_P18=squeeze(nanmean(erp_bp(1:60,13:24,times==findclosest(times,0.015)),3))-(squeeze(nanmean(erp_bp(1:60,13:24,times==findclosest(times,0.0117)),3))+squeeze(nanmean(erp_bp(1:60,13:24,times==findclosest(times,0.0192)),3)))/2;
...
erp_N20=squeeze(nanmean(erp_bp(1:60,13:24,times>0.025 & times<0.035),3));
[~,~,~,stats]=ttest(erp_N20',0);
erp_N20=mean(erp_N20,2);
erp_N20tval=stats.tstat;

erp_P30=squeeze(nanmean(erp_bp(1:60,13:24,times>0.045 & times<0.055),3)); %-squeeze(nanmean(erp_bp(1:60,:,times>0.025 & times<0.035),3));
...
plot(times,squeeze(nanmean(erp_bp(1:60,13:24,:),2)),'r'); xlim([-0.01 0.1])
xlabel('Time from onset (s)'); ylabel('ERP (\muV)'); ylim([-100 50])
