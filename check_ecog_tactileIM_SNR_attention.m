...
ylim([-2 5])
    ...
    figure;
    set(gcf,'position',[-442        1399        1075         458],'Name',sprintf('%s - %s',version,'Vertical Bi-Polar UNATTENDED'))
    subplot(2,2,1)
    [h, pV, ~, stats]=ttest(squeeze(pow_tag(sizeH+1:sizeH+sizeV,7:12,1:2)),0,'dim',2);
    imagesc(1:elecC+1,1:elecR,reshape(mean(stats.tstat,3),elecC+1,elecR)'); set(gca,'ydir','normal');
    caxis([-8 8]); colormap(cmap); colorbar;
    title('Freq Tag Contra')
    xlim([0.5 elecC + 1.5])
    ...
    subplot(2,2,2)
    [h, pV, ~, stats]=ttest(squeeze(pow_IM(sizeH+1:sizeH+sizeV,7:12,1)),0,'dim',2);
    imagesc(1:elecC+1,1:elecR,reshape(mean(stats.tstat,3),elecC+1,elecR)'); set(gca,'ydir','normal');
    caxis([-8 8]); colormap(cmap); colorbar;
    title('IM Tag Contra')
    xlim([0.5 elecC + 1.5])
    ...
    subplot(2,2,3)
    [h, pV, ~, stats]=ttest(squeeze(pow_tag(sizeH+1:sizeH+sizeV,7:12,3:4)),0,'dim',2);
    imagesc(1:elecC+1,1:elecR,reshape(mean(stats.tstat,3),elecC+1,elecR)'); set(gca,'ydir','normal');
    caxis([-8 8]); colormap(cmap); colorbar;
    title('Freq Tag Ipsi')
    xlim([0.5 elecC + 1.5])
    ...
    subplot(2,2,4)
    [h, pV, ~, stats]=ttest(squeeze(pow_IM(sizeH+1:sizeH+sizeV,7:12,2)),0,'dim',2);
    imagesc(1:elecC+1,1:elecR,reshape(mean(stats.tstat,3),elecC+1,elecR)'); set(gca,'ydir','normal');
    caxis([-8 8]); colormap(cmap); colorbar;
    title('IM Tag Ipsi')
    xlim([0.5 elecC + 1.5])
