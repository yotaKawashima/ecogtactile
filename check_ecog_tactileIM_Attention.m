...
% % %     end
    ...
    %Divide to attention effect
    att_1 = pow_bych(:,1:6,:); %condition 1 in epoch are numbered 1 till 6
    att_3 = pow_bych(:,6:12,:); %condition 3 in epoch are numbered 6 till 12
    
    %%
    figure;
    set(gcf,'position',[-499 801 854 1056],'Name',version)
    subplot(8,2,1)
    plot(faxis,log(squeeze(mean(mean(att_1([find(ismember(reref_mat(:,2),S1channels) & reref_mat(:,3)==0)],:,:),1),2))),'Color','k','LineWidth',2)
    title('Attend_1 H ref');
...
% %     ylabel(sprintf('SNR (ch %g %g) bipolar I',S1channels(1),S1channels(2)))
    ...
    subplot(8,2,2)
    plot(faxis,log(squeeze(mean(mean(att_3([find(ismember(reref_mat(:,2),S1channels) & reref_mat(:,3)==0)],:,:),1),2))),'Color','k','LineWidth',2)
    title('Attend_3 H ref');
%   plot(faxis,log(squeeze(mean(mean(pow_bych([find(ismember(reref_mat(:,2),S1channels) & reref_mat(:,3)==0)],:,:),1),2))),'Color','k','LineWidth',2)
    xlim([1 45])
    xlabel('Freq (Hz)')
    ylabel('logPower')
...
%     plot(faxis,log(squeeze(mean(mean(pow_bych([find(ismember(reref_mat(:,2),S1channels) & reref_mat(:,3)==1)],:,:),1),2))),'Color','k','LineWidth',2)
    plot(faxis,log(squeeze(mean(mean(att_1([find(ismember(reref_mat(:,2),S1channels) & reref_mat(:,3)==1)],:,:),1),2))),'Color','k','LineWidth',2)
    title('Attend_1 V ref');
    xlim([1 45])
    xlabel('Freq (Hz)')
    ylabel('logPower')
% %     ylabel(sprintf('SNR (ch %g %g) bipolar II',S1channels(1),S1channels(2)))
    ...
    subplot(8,2,4)
%     plot(faxis,log(squeeze(mean(mean(pow_bych([find(ismember(reref_mat(:,2),S1channels) & reref_mat(:,3)==1)],:,:),1),2))),'Color','k','LineWidth',2)
    plot(faxis,log(squeeze(mean(mean(att_3([find(ismember(reref_mat(:,2),S1channels) & reref_mat(:,3)==1)],:,:),1),2))),'Color','k','LineWidth',2)
    title('Attend_3 V ref');
    xlim([1 45])
    xlabel('Freq (Hz)')
    ylabel('logPower')
% %     ylabel(sprintf('SNR (ch %g %g) bipolar II',S1channels(1),S1channels(2)))
...
%         pow_tag(:,:,nfreq)=log(pow_bych(:,:,findfreq))-1/2*(log(pow_bych(:,:,findfreq-1)) + log(pow_bych(:,:,findfreq+1)));
        pow_tag_att1(:,:,nfreq)=log(att_1(:,:,findfreq))-1/2*(log(att_1(:,:,findfreq-1)) + log(att_1(:,:,findfreq+1)));
        pow_tag_att3(:,:,nfreq)=log(att_3(:,:,findfreq))-1/2*(log(att_3(:,:,findfreq-1)) + log(att_3(:,:,findfreq+1)));
...
%         pow_IM(:,:,nfreq)=log(pow_bych(:,:,findfreq))-1/2*(log(pow_bych(:,:,findfreq-1)) + log(pow_bych(:,:,findfreq+1)));
        pow_IM_att1(:,:,nfreq)=log(att_1(:,:,findfreq))-1/2*(log(att_1(:,:,findfreq-1)) + log(att_1(:,:,findfreq+1)));
        pow_IM_att3(:,:,nfreq)=log(att_3(:,:,findfreq))-1/2*(log(att_3(:,:,findfreq-1)) + log(att_3(:,:,findfreq+1)));
...
%     posX=repmat(1:elecC,1,6);
    ...
    % for nfreq=1:4
    [h, pV, ~, stats]=ttest(squeeze(pow_tag_att1(1:sizeH,:,1:2)),0,'dim',2);
...
%     ylim([0 7])
    ...
    % titles={sprintf('f0=%g (contra)',LeftF0(1)),sprintf('f0=%g (contra)',LeftF0(2)),sprintf('f0=%g (ipsi)',RightF0(1)),sprintf('f0=%g (ipsi)',RightF0(2))};
    % for nfreq=1:4
    [h, pV, ~, stats]=ttest(squeeze(pow_tag_att3(1:sizeH,:,1:2)),0,'dim',2);
    tp=mean(stats.tstat,3)/3.5*255; %((tp)/maxtp*511+1);
    tp1=tp; tp1(tp1<0)=0;
    tp2=tp; tp2(tp2>0)=0; tp2=abs(tp2);
    scatter(posX,posY,abs(tp),[round(tp1/max(tp1)*255) zeros(length(tp),1) round(tp2/max(tp2)*255)],'filled');
...
%     ylim([0 7])
    ...
    % for nfreq=1:2
    [h, pV, ~, stats]=ttest(squeeze(pow_IM_att1(1:sizeH,:,1))',0);
    % tp=((tp)/maxtp*511+1);
...
%     ylim([0 7])
    ...
    % titles={sprintf('IM=%g (contra)',LeftF0(1)+LeftF0(2)),sprintf('IM=%g (ipsi)',RightF0(1)+RightF0(2))};
    % for nfreq=1:2
    [h, pV, ~, stats]=ttest(squeeze(pow_IM_att3(1:sizeH,:,1))',0);
    % tp=((tp)/maxtp*511+1);
    tp=stats.tstat'/2*255; %((tp)/maxtp*511+1);
    tp1=tp; tp1(tp1<0)=0;
    tp2=tp; tp2(tp2>0)=0; tp2=abs(tp2);
...
%     posY=[ones(1,10) 2*ones(1,10) 3*ones(1,10) 4*ones(1,10) 5*ones(1,10)];
    ...
    % titles={sprintf('f0=%g (contra)',LeftF0(1)),sprintf('f0=%g (contra)',LeftF0(2)),sprintf('f0=%g (ipsi)',RightF0(1)),sprintf('f0=%g (ipsi)',RightF0(2))};
    % for nfreq=1:4
    [h, pV, ~, stats]=ttest(squeeze(pow_tag_att1(1:sizeH,:,3:4)),0,'dim',2);
    tp=mean(stats.tstat,3)/3.5*255; %((tp)/maxtp*511+1);
    tp1=tp; tp1(tp1<0)=0;
    tp2=tp; tp2(tp2>0)=0; tp2=abs(tp2);
    scatter(posX,posY,abs(tp),[round(tp1/max(tp1)*255) zeros(length(tp),1) round(tp2/max(tp2)*255)],'filled');
    title('Freq Tag Att 1 Ipsi')
%     xlim([0 11])
%     ylim([0 6])
     xlim([0 elecC + 2])
     ylim([0 elecR + 2])
    ...
    % titles={sprintf('f0=%g (contra)',LeftF0(1)),sprintf('f0=%g (contra)',LeftF0(2)),sprintf('f0=%g (ipsi)',RightF0(1)),sprintf('f0=%g (ipsi)',RightF0(2))};
    % for nfreq=1:4
    [h, pV, ~, stats]=ttest(squeeze(pow_tag_att3(1:sizeH,:,3:4)),0,'dim',2);
    tp=mean(stats.tstat,3)/3.5*255; %((tp)/maxtp*511+1);
    tp1=tp; tp1(tp1<0)=0;
    tp2=tp; tp2(tp2>0)=0; tp2=abs(tp2);
    scatter(posX,posY,abs(tp),[round(tp1/max(tp1)*255) zeros(length(tp),1) round(tp2/max(tp2)*255)],'filled');
    title('Freq Tag Att3 Ipsi')
%     xlim([0 11])
%     ylim([0 6])
     xlim([0 elecC + 2])
     ylim([0 elecR + 2])
    ...
    % titles={sprintf('IM=%g (contra)',LeftF0(1)+LeftF0(2)),sprintf('IM=%g (ipsi)',RightF0(1)+RightF0(2))};
    % for nfreq=1:2
    [h, pV, ~, stats]=ttest(squeeze(pow_IM_att1(1:sizeH,:,2))',0);
    % tp=((tp)/maxtp*511+1);
    tp=stats.tstat'/2*255; %((tp)/maxtp*511+1);
    tp1=tp; tp1(tp1<0)=0;
    tp2=tp; tp2(tp2>0)=0; tp2=abs(tp2);
...
%     ylim([0 6])
    ...
    % titles={sprintf('IM=%g (contra)',LeftF0(1)+LeftF0(2)),sprintf('IM=%g (ipsi)',RightF0(1)+RightF0(2))};
    % for nfreq=1:2
    [h, pV, ~, stats]=ttest(squeeze(pow_IM_att3(1:sizeH,:,2))',0);
    % tp=((tp)/maxtp*511+1);
    tp=stats.tstat'/2*255; %((tp)/maxtp*511+1);
    tp1=tp; tp1(tp1<0)=0;
    tp2=tp; tp2(tp2>0)=0; tp2=abs(tp2);
...
%     ylim([0 6])
    ...
    subplot(8,2,13)
    simpleBarPlot(1-0.2,squeeze(mean(mean(pow_tag_att1(myChans,:,1),3),1)),'b',0.38,'r',[],2);
    simpleBarPlot(1+0.2,squeeze(mean(mean(pow_tag_att1(myChans,:,2),3),1)),'b',0.38,'r',[],2);
    
    simpleBarPlot(2-0.2,squeeze(mean(mean(pow_tag_att1(myChans,:,3),3),1)),'k',0.38,'r',[],2);
    simpleBarPlot(2+0.2,squeeze(mean(mean(pow_tag_att1(myChans,:,4),3),1)),'k',0.38,'r',[],2);
    
    
    simpleBarPlot(3-0.2,squeeze(mean(pow_IM_att1(myChans,:,1),1)),'b',0.38,'r',[],2);
    simpleBarPlot(3+0.2,squeeze(mean(pow_IM_att1(myChans,:,2),1)),'k',0.38,'r',[],2);
    ...
    myChans=find(ismember(reref_mat(:,2),S1channels) & reref_mat(:,3)==0);
    subplot(8,2,14)
    simpleBarPlot(1-0.2,squeeze(mean(mean(pow_tag_att3(myChans,:,1),3),1)),'b',0.38,'r',[],2);
    simpleBarPlot(1+0.2,squeeze(mean(mean(pow_tag_att3(myChans,:,2),3),1)),'b',0.38,'r',[],2);
    
    simpleBarPlot(2-0.2,squeeze(mean(mean(pow_tag_att3(myChans,:,3),3),1)),'k',0.38,'r',[],2);
    simpleBarPlot(2+0.2,squeeze(mean(mean(pow_tag_att3(myChans,:,4),3),1)),'k',0.38,'r',[],2);
    
    
    simpleBarPlot(3-0.2,squeeze(mean(pow_IM_att3(myChans,:,1),1)),'b',0.38,'r',[],2);
    simpleBarPlot(3+0.2,squeeze(mean(pow_IM_att3(myChans,:,2),1)),'k',0.38,'r',[],2);
