...
for nversion=1:5
  ...
  erpbp(nch,ntr,:)=abs(hilbert(bandpass(squeeze(erp(nch,ntr,:)), SR, 60, 90, 4)));
    ...
    format_fig; plot(times,squeeze(mean(erpbp2(11,:,:),2))'); xlim([-0.2 1]); line([0 0],ylim,'Color','r'); line([10 10],ylim,'Color','r');
    ...
    figure;
    [~,~,~,stats]=ttest(squeeze((erpbp2(1:60,:,idxtime)))',0);
    imagesc(1:10,1:6,reshape(stats.tstat,10,6)'); set(gca,'ydir','normal');
    caxis([-8 8]); colormap(cmap); colorbar;
    title('Amplitude ERP')
   
    figure;
    set(gcf,'Position',[360   278   880   420])
    format_fig; plot(times,squeeze(mean(erpbp(11,:,:),2))'); xlim([-0.2 1]); line([0 0],ylim,'Color','r'); line([10 10],ylim,'Color','r');
    xlim([-0.8 10.8]); line([0 0],ylim,'Color','r'); line([10 10],ylim,'Color','r');
    xlabel('time (s)'); 
    format_fig
    ylabel('Gamma envelope')
    hold on; plot(times,fastsmooth(squeeze(mean(erpbp(11,:,:),2))',500,3,1),'Color','r','LineWidth',4)
...
pow_bych2=[];
    ...
    plot(faxis,squeeze(mean(snr_bych(1:60,1:6,:),2))','Color','g')
    plot(faxis,squeeze(mean(snr_bych(1:60,7:12,:),2))','Color','k')
    c=0;
    
    for n=theseFreqs
        c=c+1;
        [~,idxfreqs(c)]=findclosest(faxis,n);
        tp=squeeze(mean(snr_bych(1:60,:,idxfreqs(c)),2));
        if fillCode(c)==1
...
ylim([-2 5])
    ...
    subplot(2,2,1)
    [h, pV, ~, stats]=ttest(squeeze(pow_tag(1:60,1:6,1:2)),0,'dim',2);
    imagesc(1:10,1:6,reshape(mean(stats.tstat,3),10,6)'); set(gca,'ydir','normal');
    caxis([-8 8]); colormap(cmap); colorbar;
    title('Freq Tag Contra')

    subplot(2,2,3)
    [h, pV, ~, stats]=ttest(squeeze(pow_IM(1:60,1:6,1)),0,'dim',2);
    imagesc(1:10,1:6,reshape(mean(stats.tstat,3),10,6)'); set(gca,'ydir','normal');
    caxis([-8 8]); colormap(cmap); colorbar;
    title('IM Tag Contra')
    
    subplot(2,2,2)
    [h, pV, ~, stats]=ttest(squeeze(pow_tag(1:60,1:6,3:4)),0,'dim',2);
    imagesc(1:10,1:6,reshape(mean(stats.tstat,3),10,6)'); set(gca,'ydir','normal');
    caxis([-8 8]); colormap(cmap); colorbar;
    title('Freq Tag Ipsi')
    
    subplot(2,2,4)
    [h, pV, ~, stats]=ttest(squeeze(pow_IM(1:60,1:6,2)),0,'dim',2);
    imagesc(1:10,1:6,reshape(mean(stats.tstat,3),10,6)'); set(gca,'ydir','normal');
    caxis([-8 8]); colormap(cmap); colorbar;
    title('IM Tag Ipsi')   
    ...
    subplot(2,2,1)
    [h, pV, ~, stats]=ttest(squeeze(pow_tag(1:60,7:12,1:2)),0,'dim',2);
    imagesc(1:10,1:6,reshape(mean(stats.tstat,3),10,6)'); set(gca,'ydir','normal');
    caxis([-8 8]); colormap(cmap); colorbar;
    title('Freq Tag Contra')
    
    subplot(2,2,3)
    [h, pV, ~, stats]=ttest(squeeze(pow_IM(1:60,7:12,1)),0,'dim',2);
    imagesc(1:10,1:6,reshape(mean(stats.tstat,3),10,6)'); set(gca,'ydir','normal');
    caxis([-8 8]); colormap(cmap); colorbar;
    title('IM Tag Contra')

    subplot(2,2,2)
    [h, pV, ~, stats]=ttest(squeeze(pow_tag(1:60,7:12,3:4)),0,'dim',2);
    imagesc(1:10,1:6,reshape(mean(stats.tstat,3),10,6)'); set(gca,'ydir','normal');
    caxis([-8 8]); colormap(cmap); colorbar;
    title('Freq Tag Ipsi')
    ...
    subplot(2,2,4)
    [h, pV, ~, stats]=ttest(squeeze(pow_IM(1:60,7:12,2)),0,'dim',2);
    imagesc(1:10,1:6,reshape(mean(stats.tstat,3),10,6)'); set(gca,'ydir','normal');
    caxis([-8 8]); colormap(cmap); colorbar;
    title('IM Tag Ipsi')
