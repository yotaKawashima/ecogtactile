...
if(sd)
    ...
    if (useMac)
        ...
        rootFile    = 'C:/Sharon/PhdAust/Matlab/';
        chronuxPath = 'C:\Program Files\MATLAB\R2017b\toolbox\chronux_2_12';
        LSCPPath    = 'C:\Program Files\MATLAB\R2017b\toolbox\LSCPtools';
        addpath(genpath('C:\Program Files\MATLAB\R2017b\toolbox\SharonDependencies\stats'));
    end %useMac
...
% %      
% att_1 = mean(max(erp2(:,1:6, :),[],3), 2); %Avarge of max in time between 6 same conditions
% att_3 = mean(max(erp2(:,6:end, :),[],3), 2);
...
figure;
subplot(2,1,1)
plot(times,squeeze(mean(erp2(:,1:6,:),2)),'r'); xlim([-0.05 0.4])    % Attend_1
hold on;
plot(times,squeeze(mean(erp2(:,7:end,:),2)),'k'); xlim([-0.05 0.4])   %Attend_3
subplot(2,1,2)
plot(times,squeeze(mean(erp2(:,1:6,:),2)-mean(erp2(:,7:end,:),2)),'Color',[1 1 1]*0.7); xlim([-0.05 0.4])    % Attend_1
