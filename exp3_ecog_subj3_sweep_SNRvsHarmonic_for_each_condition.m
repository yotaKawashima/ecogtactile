%% 
% Plot relation between SNR vs harmonic frequency of all bipolar-channels
...
path_save_dir_exp3=[path_save_dir, filesep, 'Exp3', filesep, 'harmonic_frequency_range_vs_SNR_condition', filesep];
...
for nversion=1:length(file_name_prefixs)
    ...
    for frequency_id = 1:2
        ...
        % Remove 50Hz 
        target_frequency_all_array_without_50 = target_frequency_all_array(mod(target_frequency_all_array,50) ~= 0);
        
        nStimuli_frequency = 21; 
        
        % frequency description for figure
        switch sweep_type 
        case 'A'
            freq_description = strcat(num2str(target_frequency_array(1)),':+', num2str(frequency_id), ':', num2str(target_frequency_array(end)), ' Hz');
        case 'D'
            freq_description = strcat(num2str(target_frequency_array(1)),':-', num2str(frequency_id), ':', num2str(target_frequency_array(end)), ' Hz');
        end
        ...
        % Data storage
        store_array_mean_for_plot_harmonic = zeros(nHarmonic, size(SNRdB, 1)); % dim = range x num. of bipolar-channel
        store_array_std_for_plot_harmonic = zeros(nHarmonic, size(SNRdB, 1));
        ...
        for bipolar_ch_id = 1:size(SNRdB,1)
            ...
            for data_type=0:1
                if data_type == 0
                    ...
                    fig_file_prefix_multiple = strcat('relation_frequency_multiple_PowdB_');
                    fig_file_prefix_range = strcat('relation_frequency_range_PowdB_');
                    plot_txt = 'log10(Power) [-]';
                    % Prepare a figure window for all ch plot 
                    if bipolar_ch_id == 1
                        f_pow_harmonic = figure('visible','off');
                        f_pow_range = figure('visible','off');
                    end
                elseif data_type == 1
                    logdata_this_ch = squeeze(SNRdB(bipolar_ch_id,:,:)); % dim = faxis(frequecny) x epochs
                    fig_file_prefix_multiple = strcat('relation_frequency_multiple_SNRdB_');
                    fig_file_prefix_range = strcat('relation_frequency_range_SNRdB_');
                    plot_txt = '10log10(SNR) [dB]';
                    % Prepare a figure window for all ch plot
                    if bipolar_ch_id == 1
                        f_snr_harmonic = figure('visible','off');
                        f_snr_range = figure('visible','off');
                    end
                end

                %%% Plot relation frequency and logPow or log SNR
                fig_name_harmonic = strcat(file_name_prefix, '_harmonic_', string(bipolar_ch_id), 'ch');
                fig_name_range = strcat(file_name_prefix, '_range_', string(bipolar_ch_id), 'ch');

                %%% Harmonic
                for harmonic_id = 1:nHarmonic
                    store_array_for_mean_harmonic = zeros(nStimuli_frequency,1);
                    for trial_id = 1:nStimuli_frequency 
                        current_frequency = target_frequency_all_array(trial_id + nStimuli_frequency*(harmonic_id - 1));
                        if mod(current_frequency, 50) == 0 
                            store_array_for_mean_harmonic(trial_id) = nan;
                        else
                            [~, frequency_id_in_array] = findclosest(faxis, current_frequency);
                            current_logdata = logdata_this_ch(frequency_id_in_array, trial_id);
                            if data_type == 0 
                                current_logdata = (current_logdata / 10);
                            end
                            store_array_for_mean_harmonic(trial_id) = current_logdata;
                    
                        end
                    end
                    
                    store_array_for_mean_harmonic(isnan(store_array_for_mean_harmonic)) =[];
                    
                    store_array_mean_for_plot_harmonic(harmonic_id, bipolar_ch_id) = mean(store_array_for_mean_harmonic);
                    store_array_std_for_plot_harmonic(harmonic_id, bipolar_ch_id) = std(store_array_for_mean_harmonic);
                end

                % Figure harmonic for each ch  
                
                f_temp = figure('Name', fig_name_harmonic, 'visible','off');
                errorbar(1:harmonic_id, store_array_mean_for_plot_harmonic(:, bipolar_ch_id), store_array_std_for_plot_harmonic(:, bipolar_ch_id), 'o', 'MarkerSize', 15, 'MarkerEdgeColor', 'auto', 'MarkerFaceColor', 'auto', 'LineWidth', 3, 'CapSize', 15);
                %{
                hold on;
                for harmonic_id = 1:nHarmonic
                ...
                xticklabels({'1f','2f','3f','4f','5f','6f', '7f', '8f'});
                xlabel('Harmonic frequency [Hz]');
                ylabel(plot_txt);
                title(['(f1,f2) = (', finger1, ',', finger2, ')', ', sweep type : ', sweep_name, ', Target f : ', 'f', num2str(frequency_id), '=', freq_description]);
                format_fig;
                set(gcf, 'Position', get(0, 'Screensize'));
                saveas(gcf, strcat(path_save_dir_exp3, fig_file_prefix_multiple, file_name_prefix, '_f', num2str(frequency_id), '_', num2str(bipolar_ch_id) ,'ch.png'));
                close(f_temp);
                ...
                if data_type == 0
                    figure(f_pow_harmonic);
                else
                    figure(f_snr_harmonic);
                end
                ...
                if bipolar_ch_id == size(SNRdB,1)
                    ...
                    sgtitle(['(f1,f2) = (', finger1, ',', finger2, ')', ', Sweep type : ', sweep_name, ', Target : ', 'f', num2str(frequency_id), '=', freq_description], 'FontSize', 20, 'FontWeigh', 'bold');
                    format_fig;
                    set(gcf, 'Position', get(0, 'Screensize'));
                    saveas(gcf, strcat(path_save_dir_exp3, fig_file_prefix_multiple, file_name_prefix, '_f', num2str(frequency_id), '_allch.png'));
                    close gcf;
                end 
                ...
                for range_id = 1:size(frequency_range,1)
                    ...
                    frequency_indices = find(target_frequency_all_array_without_50 > min_frequency & (target_frequency_all_array_without_50 < max_frequency | target_frequency_all_array_without_50 == max_frequency));
                    store_array_for_mean_range = zeros(length(frequency_indices),1);
                    for frequency_now = 1:length(frequency_indices)
                        ...
                        store_array_for_mean_range(frequency_now) = current_logdata;
                    end
                ...
                ylabel(plot_txt);
                title(['(f1,f2) = (', finger1, ',', finger2, ')', ', Sweep type : ', sweep_name, ', Target : ', 'f', num2str(frequency_id), '=', freq_description]);
                format_fig;
                set(gcf, 'Position', get(0, 'Screensize'));
                saveas(gcf, strcat(path_save_dir_exp3, fig_file_prefix_range, file_name_prefix, '_f', num2str(frequency_id) , '_', num2str(bipolar_ch_id) ,'ch.png'));
                close(f_temp);
                ...
                if bipolar_ch_id == size(SNRdB,1)
                    ...
                    format_fig;
                    set(gcf, 'Position', get(0, 'Screensize'));
                    saveas(gcf, strcat(path_save_dir_exp3, fig_file_prefix_range, file_name_prefix, '_f', num2str(frequency_id) , '_allch.png'));
                    close gcf;
                end 
